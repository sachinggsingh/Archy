...
func main() {
    ...
    e.GET("/health",func(c echo.Context)error{
        return c.JSON(http.StatusOK, fmt.Sprintf("Hello from %s", "$SERVICE_NAME"))
    })
    port := $((8080 + i))
...
do
    ...
    curl -s http://localhost:\$PORT/health | grep "service-1" && echo "service-\$i is UP" || echo "service-\$i is DOWN"
done
EOF
    chmod +x test.sh
fi